...
def modelling(input_steps,
    ...
    # Amount input branch
    input_amount = tf.keras.layers.Input(shape=(input_steps,), dtype='float32', name="amount_input")
    amount = tf.keras.layers.Reshape(target_shape=(input_steps, 1))(input_amount)

    # "concat" mode can only merge layers with matching output shapes except for the concat axis.
    concat = tf.keras.layers.Concatenate(axis=2)([transactions, weekday, day, amount])
    ...
    output_amount = tf.keras.layers.Dense(1, name="output_amount")(drop_2)

    model = tf.keras.Model(inputs=[input_transactions, input_weekday, input_day, input_amount],
                           outputs=[output_transaction, output_weekday, output_day, output_amount])
    return model