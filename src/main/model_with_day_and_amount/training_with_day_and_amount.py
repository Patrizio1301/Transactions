...
def training(model,
    ...
    model.compile(loss=['categorical_crossentropy', 'categorical_crossentropy','categorical_crossentropy', 'mse'],
                  optimizer=adamOpti,
    ...
    y_amount_pred = predictions[3]

    results = pd.DataFrame({'transaction': y_transactions,
                            'weekday': y_weekday,
                            'day': y_day,
                            'amount': y_amount,
                            'transaction_pred': y_transactions_pred,
                            'weekday_pred': y_weekday_pred,
                            'day_pred': y_day_pred,
                            'amount_pred': y_amount_pred.reshape(-1)})
